...
#Create a query to display all the data from the EMP table. Separate each column by acomma. Name the column THE OUTPUT
SELECT CONCAT_WS(', ', first_name, job_id, hire_date, salary, manager_id, department_id) AS 'THE OUTPUT'
FROM employees;
...
SELECT
    first_name AS Employee_Name,
    salary AS Salary,
    IFNULL(commission_pct * 2, 'No commission') AS Commission_pct
FROM
    employees;
...
FROM
    employees e1
JOIN
    employees e2 ON e1.first_name = e2.last_name
...
SELECT
    ...
    department_id AS Manager_Department_ID
FROM
    employees 
JOIN
...
SELECT
    ...
    CONCAT(UPPER(SUBSTRING(last_name, 2, 1)), LOWER(SUBSTRING(last_name, 3))) AS Formatted_Last_Name
FROM
    employees;
...
SELECT
    ...
    REPLACE(UPPER(manager_id), 'Z', '$!') AS Formatted_Manager_Name
FROM
    employees;
...
FROM
    employees
WHERE
    REPLACE(LOWER(first_name), ' ', '') = REVERSE(REPLACE(LOWER(last_name), ' ', ''));
...
SELECT
    SUBSTRING_INDEX(SUBSTRING_INDEX(`street_address`, ' ', 2), ' ', -1) AS Word_Between_First_And_Second_Space
FROM
    LOCATIONS;
...
SELECT
    ...
    CONCAT(LOWER(LEFT(first_name, 1)), last_name, '@systechusa.com') AS Email_Address
FROM
    employees;
...
SELECT e1.last_name AS employee_last_name,
       e1.department_id AS employee_department_number,
       e2.last_name AS colleague_last_name,
       e2.department_id AS colleague_department_number
FROM employees e1
